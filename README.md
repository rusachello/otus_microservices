...
Добавим хук pre-commit (проверяет, нет ли лишних пробелов в наших файлах и есть ли в конце файла пустая строка)
	...
	2. Установить pre-commit (pip install pre-commit)
	3. Добавить .pre-commit-config.yaml в корень репозитория
	4. pre-commit install
mkdir .github && cd .github и добавить PULL_REQUEST_TEMPLATE.md
Настроить интеграцию текущего репозитория (GitHub) со Slack (https://slack.com/intl/en-ru/help/articles/232289568)
	В слаке на канале интеграций:
	```
	/github subscribe  rusachello/otus_microservices commits:all
	```

Travis CI
Интеграция Slack c Travis CI для отправки сообщений в Slack
	в Slack -> Add more apps -> Travis CI -> View in App Directory -> Add to Slack -> скопировать содержимое в "Simple notifications"
	добавить .travis.yml в корень репозитория и вставить содержимое из "Simple notifications"
	travis encrypt "z585:<ваш_токен>" --add notifications.slack --com # шифрование токена для интеграции со Slack

Также необходимо убедиться, что в настройках GitHub -> Applications установлены настроены Slack и Travis CI

## Установка докер/композ/машин по гайдам с https://docs.docker.com/:
```
